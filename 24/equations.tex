...
This leaves us with 2 equations and 6 unknowns: $X_{r0}$, $Y_{r0}$, $Z_{r0}$, $V_{rx}$, $V_{ry}$ and $V_{rz}$. For us to be able to
find these unknowns, we require 4 more equations. Fortunately there are more than enough points to plug in, just take 2 more hailstones.

Unfortunately this does not look like Linear equation system, so simple solution using \emph{System of Linear equations} is not likely to be viable.

I'm giving up for now and resorting to equation solver-based solution. Let's try out Z3.
